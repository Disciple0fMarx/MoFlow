...
class SDDDataset(Dataset):
    ...
    def _load_sdd_trajectories(self, data_root, training, subset=None):
        ...
        if subset is not None:
            scenes_to_use = [subset] if subset in SDD_SCENES else SDD_SCENES
        else:
            scenes_to_use = SDD_SCENES
        ...
        for scene in scenes_to_use:
            ...
            # Get list of videos for this scene
            video_dirs = sorted([d for d in os.listdir(scene_annotations_dir) 
            ...
            for video_id in video_dirs:
                ...
                # Group by track ID (column 0)
                track_ids = np.unique(annotations[:, 0].astype(int))
                
                for track_id in track_ids:
                    track_data = annotations[annotations[:, 0].astype(int) == track_id]
                    
                    # Sort by frame number (column 5)
        ...
        if len(all_trajectories) == 0:
            raise ValueError(f"No trajectories found in {data_root}!")
